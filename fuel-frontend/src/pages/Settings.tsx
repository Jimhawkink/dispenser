    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          ...
          <div key={f.key} className="flex items-center justify-between p-3 rounded-xl bg-gray-800/40 border border-gray-700/30">
            <span className="text-sm text-gray-300">{f.label}</span>
            <button onClick={() => set(f.key, form[f.key] === 'true' ? 'false' : 'true')}
              className={`w-10 h-5 rounded-full transition-all relative ${form[f.key] === 'true' ? 'bg-emerald-500' : 'bg-gray-600'}`}>
      ...
      {showAdd && (
        <motion.form initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} onSubmit={handleAdd} className="glass-card p-5 border border-emerald-500/20 space-y-3">
          ...
          <div className="flex items-center gap-3">
            ...
            <label htmlFor="primary" className="text-sm text-gray-300">Set as primary account</label>
          </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="flex items-center gap-2">
                  <p className="font-medium text-white">{acc.bank_name}</p>
                  {acc.is_primary && <span className="text-xs px-2 py-0.5 rounded-full bg-emerald-500/15 text-emerald-400">Primary</span>}
                </div>
                <p className="text-sm text-gray-400">{acc.account_name}</p>
                <p className="font-mono text-xs text-gray-500 mt-0.5">{acc.account_number}{acc.branch ? ` · ${acc.branch}` : ''}</p>
                <div className="flex gap-3 mt-1">
                  {acc.paybill_number && <span className="text-xs text-gray-500">Paybill: {acc.paybill_number}</span>}
                  {acc.till_number && <span className="text-xs text-gray-500">Till: {acc.till_number}</span>}
                </div>
              </div>
            </div>
          </div>
        ))}
        {(data || []).length === 0 && <div className="text-center py-8 text-gray-500 text-sm">No bank accounts configured</div>}
      </div>
    </div>
  );
}
          ...
          <div key={u.id} className="glass-card p-4 flex items-center justify-between">
            ...
            {currentUser?.role === 'admin' && u.id !== currentUser.id && (
              <button onClick={() => toggleActive(u.id, !u.is_active)}
                className={`text-xs px-3 py-1.5 rounded-lg border transition-all ${u.is_active ? 'border-gray-600 text-gray-400 hover:border-red-500/50 hover:text-red-400' : 'border-emerald-500/30 text-emerald-400 hover:bg-emerald-500/10'}`}>
...
export default function Settings() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex gap-1 p-1 bg-gray-900/60 rounded-xl w-fit flex-wrap">
        {TABS.map(t => (
          <button key={t.id} onClick={() => setTab(t.id as typeof tab)}
            className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-all ${tab === t.id ? 'bg-emerald-500 text-white' : 'text-gray-400 hover:text-white'}`}>
