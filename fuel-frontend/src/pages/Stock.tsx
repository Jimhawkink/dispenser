    ...
    <div className="modal-overlay" onClick={e => e.target === e.currentTarget && onClose()}>
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="glass-card w-full max-w-md">
        <div className="flex items-center justify-between p-5 border-b border-gray-800/50">
          <h3 className="font-heading font-semibold text-white flex items-center gap-2"><Truck size={16} className="text-emerald-400" /> Record Delivery</h3>
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
          ...
          {totalCost > 0 && (
            <div className="p-3 rounded-xl bg-emerald-500/10 border border-emerald-500/20">
              <div className="flex justify-between text-sm"><span className="text-gray-400">Total Cost</span><span className="text-emerald-400 font-bold num">{formatKES(totalCost)}</span></div>
              {selectedProduct && <div className="flex justify-between text-xs mt-1"><span className="text-gray-500">New Stock</span><span className="text-gray-300 num">{formatLitres(selectedProduct.current_stock_litres + parseFloat(form.litres_delivered || '0'))}</span></div>}
            </div>
          )}
          <div className="grid grid-cols-2 gap-3">
      ...
      {lowStock.length > 0 && (
        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="p-4 rounded-xl bg-red-500/10 border border-red-500/30 flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-gray-400 text-xs mt-0.5">These products are at or below their alert thresholds. Schedule a delivery immediately.</p>
          </div>
        </motion.div>
      )}
              ...
              <motion.div key={p.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }}
                ...
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-mono text-xs text-gray-400 bg-gray-800/60 px-2 py-0.5 rounded">{p.code}</span>
                  </div>
                  {(isLow || isCritical) && (
                ...
                <div className="mt-4 pt-4 border-t border-gray-800/50 grid grid-cols-2 gap-3 text-sm">
                  <div><p className="text-gray-500 text-xs">Buying Price</p><p className="text-gray-200 font-medium num">KES {p.buying_price_per_litre.toFixed(2)}</p></div>
                  <div><p className="text-gray-500 text-xs">Selling Price</p><p className="text-emerald-400 font-bold num">KES {p.selling_price_per_litre.toFixed(2)}</p></div>
                </div>
              </motion.div>
            );
          })}
              ...
              <tbody>
                {(deliveries || []).map((d, i) => (
                  <motion.tr key={d.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.03 }} className="table-row">
                    <td className="table-cell text-xs text-gray-400 whitespace-nowrap">{format(new Date(d.delivery_date), 'dd MMM yyyy')}</td>
                    <td className="table-cell">
                      <div className="flex items-center gap-2">
                        <div className="w-2 h-2 rounded-full" style={{ background: (d.product as Product)?.colour || '#10b981' }} />
                        <span className="text-sm text-white">{(d.product as Product)?.name || '—'}</span>
                      </div>
                    </td>
                    <td className="table-cell text-right font-bold text-emerald-400 num">{formatLitres(d.litres_delivered)}</td>
                    <td className="table-cell text-right text-sm num text-gray-300">KES {d.cost_per_litre.toFixed(2)}</td>
                    <td className="table-cell text-right font-bold text-sm num text-white">{formatKES(d.total_cost)}</td>
                    <td className="table-cell hidden md:table-cell text-xs text-gray-400">{d.supplier_name || '—'}</td>
                    <td className="table-cell hidden lg:table-cell text-xs font-mono text-gray-500">{d.invoice_number || '—'}</td>
                  </motion.tr>
                ))}
              </tbody>
            </table>
            {(deliveries || []).length === 0 && <div className="text-center py-10 text-gray-500 text-sm">No deliveries recorded yet</div>}
          </div>
        )}
      </div>
