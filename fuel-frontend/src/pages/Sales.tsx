      ...
      <div className="grid grid-cols-1 xl:grid-cols-5 gap-6">
          ...
          <div className="glass-card p-4">
            ...
            <div className="relative">
              <div className="relative">
                ...
                {selectedCustomer && <button onClick={() => { setSelectedCustomer(null); setCustomerSearch(''); }} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-500 hover:text-red-400"><X size={14} /></button>}
              </div>
              <AnimatePresence>
                {showCustomerDrop && !selectedCustomer && (customersData || []).length > 0 && (
                  <motion.div initial={{ opacity: 0, y: -5 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -5 }}
                    className="absolute z-20 top-full mt-1 w-full glass-card border border-gray-700/50 py-1 max-h-52 overflow-y-auto">
                    {(customersData || []).map(c => (
            ...
            {selectedCustomer && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mt-3 p-3 rounded-xl bg-emerald-500/10 border border-emerald-500/20 flex items-center justify-between">
                <div>
                  <p className="text-sm font-semibold text-white">{selectedCustomer.name} <span className="text-xs text-emerald-400 font-mono ml-1">{selectedCustomer.customer_code}</span></p>
                  <p className="text-xs text-gray-400">{selectedCustomer.phone}{selectedCustomer.company_name ? ` · ${selectedCustomer.company_name}` : ''}</p>
                </div>
                <div className="text-right">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              {(products || []).map(p => (
                <motion.button key={p.id} whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}
                  ...
                  className="p-4 rounded-xl border border-gray-700/50 hover:border-emerald-500/40 bg-gray-800/40 hover:bg-gray-700/40 transition-all text-left group">
                  ...
                  <p className="text-emerald-400 font-bold text-lg num mt-1">KES {p.selling_price_per_litre.toFixed(2)}<span className="text-xs text-gray-500 font-normal">/L</span></p>
                  <p className="text-xs text-gray-500 mt-1">Stock: {p.current_stock_litres.toLocaleString()}L</p>
          ...
          {cartItems.length > 0 && (
            <div className="glass-card p-4">
              ...
              <div className="space-y-3">
                {cartItems.map(item => (
                  <div key={item.product_id} className="flex items-center gap-3 p-3 rounded-xl bg-gray-800/40 border border-gray-700/30">
                    <div className="flex-1">
                      <p className="text-sm font-medium text-white">{item.productName}</p>
                      <p className="text-xs text-gray-500">@ KES {item.price_per_litre.toFixed(2)}/L</p>
                    </div>
                    <div className="flex items-center gap-2">
                      <button onClick={() => updateLitres(item.product_id, item.litres - 10)} className="w-7 h-7 rounded-lg bg-gray-700 text-gray-300 hover:bg-gray-600 flex items-center justify-center text-xs font-bold">-10</button>
                      <input type="number" value={item.litres} onChange={e => updateLitres(item.product_id, parseFloat(e.target.value) || 0)}
                        className="w-20 bg-gray-800 border border-gray-600 rounded-lg text-center text-white text-sm py-1 px-2 num" />
                      <button onClick={() => updateLitres(item.product_id, item.litres + 10)} className="w-7 h-7 rounded-lg bg-gray-700 text-gray-300 hover:bg-gray-600 flex items-center justify-center text-xs font-bold">+10</button>
                    </div>
                    <p className="text-emerald-400 font-bold text-sm num w-24 text-right">{formatKES(item.subtotal)}</p>
                    <button onClick={() => setCartItems(prev => prev.filter(i => i.product_id !== item.product_id))} className="text-gray-500 hover:text-red-400"><Trash2 size={14} /></button>
                  </div>
                ))}
              </div>
            </div>
        ...
        <div className="xl:col-span-2 space-y-4">
          <div className="glass-card p-5 sticky top-4">
            ...
            <div className="flex rounded-xl overflow-hidden border border-gray-700/50 mb-4">
              {(['pay_now', 'pay_later'] as const).map(type => (
                <button key={type} onClick={() => setPaymentType(type)}
                  className={`flex-1 py-2.5 text-sm font-medium transition-all ${paymentType === type ? 'bg-emerald-500 text-white' : 'bg-gray-800/50 text-gray-400 hover:text-white'}`}>
            ...
            <div className="space-y-2 mb-4">
              <div className="flex justify-between text-sm"><span className="text-gray-400">Subtotal</span><span className="text-gray-200 num">{formatKES(total)}</span></div>
              {paymentType === 'pay_now' && (
                <div className="flex justify-between text-sm">
                  <span className="text-gray-400">Amount Paid</span>
                  <input type="number" value={amountPaid} onChange={e => setAmountPaid(e.target.value)}
                    placeholder={total.toFixed(2)} className="w-32 bg-gray-800/60 border border-gray-700/50 rounded-lg text-right text-white text-sm py-1 px-2 num" />
                </div>
              )}
              <div className="border-t border-gray-700/50 pt-2 flex justify-between font-semibold">
                <span className="text-gray-300">Total</span>
                <span className="font-heading text-2xl text-white num">{formatKES(total)}</span>
              </div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  {PAYMENT_METHODS.map(m => (
                    <button key={m.value} onClick={() => setPaymentChannel(m.value)}
                      className={`flex items-center gap-2 p-2.5 rounded-xl border text-sm transition-all ${paymentChannel === m.value ? 'border-emerald-500/50 bg-emerald-500/10 text-emerald-400' : 'border-gray-700/40 text-gray-400 hover:border-gray-600'}`}>
              ...
              <tbody>
                {(salesData || []).map((s, i) => (
                  <motion.tr key={s.id} initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.03 }} className="table-row">
                    ...
                    <td className="table-cell"><p className="text-sm text-white">{s.customer?.name || '—'}</p><p className="text-xs text-gray-500">{s.customer?.customer_code}</p></td>
                    <td className="table-cell hidden md:table-cell text-xs text-gray-400">{format(new Date(s.sale_date), 'dd MMM yyyy HH:mm')}</td>
                    <td className="table-cell text-right font-bold text-sm num text-white">{formatKES(s.total_amount)}</td>
                    <td className="table-cell text-right text-sm num text-emerald-400 hidden sm:table-cell">{formatKES(s.amount_paid)}</td>
                    <td className="table-cell text-right text-sm num hidden sm:table-cell"><span className={s.balance_due > 0 ? 'text-red-400' : 'text-gray-500'}>{formatKES(s.balance_due)}</span></td>
                    <td className="table-cell text-center"><StatusBadge status={s.payment_status} /></td>
                  </motion.tr>
                ))}
              </tbody>
            </table>
            {(salesData || []).length === 0 && <div className="text-center py-10 text-gray-500 text-sm">No sales found</div>}
          </div>
        )}
      </div>
    </div>
