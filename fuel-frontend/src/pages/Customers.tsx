...
const PAYMENT_METHODS = [
  {
    ...
    icon: Smartphone,
    desc: 'STK Push or Paybill. Auto-matched by phone or customer code.',
    autoSync: true,
  ...
  },
  {
    ...
    icon: CreditCard,
    desc: 'Inter-bank transfers. Auto-matched by account reference.',
    autoSync: true,
  ...
  },
  {
    ...
    icon: Landmark,
    desc: 'Direct bank & Equity Jenga IPN. Matched by account reference.',
    autoSync: true,
  ...
  },
  {
    ...
    icon: Banknote,
    desc: 'Customer deposits at bank branch or M-Pesa agent. Cashier confirms on bank slip.',
    autoSync: false,
  ...
  },
  {
    ...
    label: 'Cash (On-Site)',
    icon: Wallet,
    desc: 'Physical cash at station. Cashier enters manually. Instant reconciliation.',
    autoSync: false,
...
function CustomerCard({ c, onClick }: { c: Customer; onClick: () => void }) {
  ...
  return (
    ...
    >
      ...
      {balance > 0 && (
        <div className="space-y-1">
          <div className="flex justify-between text-xs">
            ...
            <span className={`font-semibold ${pct > 80 ? 'text-red-500' : 'text-slate-700'}`}>
      ...
      {!c.is_active && <div className="mt-2 text-xs text-slate-400 italic">Inactive account</div>}
    </motion.div>
  );
}
    ...
    mutationFn: (data: typeof form) => {
      const payload = {
        ...
        vehicle_plates: data.vehicle_plates
          ? data.vehicle_plates.split(',').map((s: string) => s.trim()).filter(Boolean)
          : [],
      };
    ...
    >
      ...
      >
              ...
              <div key={i} className="flex items-center gap-2">
                <button onClick={() => setStep(i)} className="flex items-center gap-1.5">
                  <div className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold transition-all ${
                    i === step ? 'bg-emerald-500 text-white' : i < step ? 'bg-emerald-100 text-emerald-600' : 'bg-slate-100 text-slate-400'
                  }`}>
                    {i < step ? <CheckCircle2 size={14} /> : i + 1}
                  </div>
                  <span className={`text-xs hidden sm:block ${i === step ? 'text-slate-700 font-medium' : 'text-slate-400'}`}>
                    {s.label}
                  </span>
                </button>
                {i < steps.length - 1 && (
            ...
            {step === 1 && (
              <motion.div key="s1" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-3">
                <p className="text-sm text-slate-600 mb-2">
                  Select how this customer usually pays. Used for <strong>auto-reconciliation</strong> of incoming payments.
                </p>
                {PAYMENT_METHODS.map(method => (
                  <button
                    ...
                    className={`w-full text-left p-4 rounded-xl border-2 transition-all ${
                      form.payment_preference === method.id ? method.activeClass : 'border-slate-200 hover:border-slate-300 bg-white'
                    }`}
                  >
                    <div className="flex items-center gap-3">
                ...
                <div className="p-4 rounded-xl bg-blue-50 border border-blue-200">
                  ...
                  <p className="text-xs text-blue-600">
                    When a payment arrives via webhook, the system matches it using the customer's <strong>phone number</strong> or <strong>customer code</strong> as the Paybill reference. Matched payments are auto-posted to the credit ledger and oldest sale settled automatically.
                  </p>
                </div>
              </motion.div>
            )}
            ...
            {step === 2 && (
              <motion.div key="s2" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-3">
                <div>
                  ...
                  <p className="text-xs text-slate-400 mt-1">Maximum outstanding balance. Set 0 to disable credit.</p>
                </div>
                <div>
                  <label className="label">Internal Notes</label>
                  <textarea value={form.notes} onChange={e => set('notes', e.target.value)} className="input-field resize-none" rows={3} placeholder="Special instructions, delivery preferences..." />
                </div>
                <div className="flex items-center justify-between p-3 rounded-xl bg-slate-50 border border-slate-200">
                  <div>
                    <p className="text-sm font-medium text-slate-700">Active Account</p>
                    <p className="text-xs text-slate-400">Inactive customers cannot make sales</p>
                  </div>
                  <button
                    type="button"
                ...
                <div className="p-4 rounded-xl bg-slate-50 border border-slate-200 space-y-1.5">
                  <p className="text-xs font-semibold text-slate-600 uppercase tracking-wider mb-2">Summary</p>
                  <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-xs">
                    ...
                    <span className="font-medium text-slate-700">KES {Number(form.credit_limit).toLocaleString()}</span>
                  </div>
                </div>
              </motion.div>
            )}
        ...
        <div className="px-6 py-4 border-t border-slate-100 flex items-center justify-between bg-slate-50">
          ...
          {step < 2 ? (
            <button onClick={() => setStep(s => s + 1)} disabled={step === 0 && !form.name} className="btn-primary py-2 px-5 text-sm">
              Next →
            </button>
          ) : (
            <button
    ...
    <div className="space-y-6">
      ...
      <div className="flex gap-2 flex-wrap">
        {PAYMENT_METHODS.map(m => {
          const count = customers.filter((c: Customer) => c.payment_preference === m.id).length;
          return count > 0 ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
          {customers.map((c: Customer, i: number) => (
            <motion.div key={c.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.04 }}>
              <CustomerCard c={c} onClick={() => navigate(`/customers/${c.id}`)} />
            </motion.div>
          ))}
          {customers.length === 0 && (
