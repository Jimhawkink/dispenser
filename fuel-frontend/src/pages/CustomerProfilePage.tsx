...
export default function CustomerProfilePage() {
  ...
  if (!customer) return <div className="text-center py-20 text-gray-500">Customer not found</div>;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        <button onClick={() => navigate(-1)} className="w-9 h-9 rounded-xl bg-gray-800/60 flex items-center justify-center text-gray-400 hover:text-white hover:bg-gray-700/60 transition-all">
          <ArrowLeft size={16} />
        </button>
        <div>
        ...
        <div className="space-y-4">
          <div className="glass-card p-5">
            <div className="flex items-center gap-4 mb-5">
              ...
              <div>
                <h2 className="font-heading font-bold text-white text-lg">{customer.name}</h2>
                {customer.company_name && <p className="text-gray-400 text-sm">{customer.company_name}</p>}
                <span className={`text-xs px-2 py-0.5 rounded-full ${customer.is_active ? 'bg-emerald-500/15 text-emerald-400' : 'bg-gray-500/15 text-gray-400'}`}>
                ...
                return (
                  <div key={i.label} className="flex items-center gap-3">
                    <div className="w-8 h-8 rounded-lg bg-gray-800/60 flex items-center justify-center flex-shrink-0">
                      <i.icon size={13} className="text-gray-400" />
                    </div>
                    <div>
                      <p className="text-[10px] text-gray-600 uppercase tracking-wide">{i.label}</p>
                      <p className="text-sm text-gray-200">{i.value}</p>
                    </div>
                  </div>
                );
              })}
            ...
            {(customer.vehicle_plates || []).length > 0 && (
              <div className="mt-4 pt-4 border-t border-gray-800/50">
                <p className="text-xs text-gray-500 uppercase tracking-wider mb-2">Vehicle Plates</p>
                <div className="flex flex-wrap gap-2">
                  {(customer.vehicle_plates || []).map(plate => (
                    <span key={plate} className="text-xs font-mono bg-gray-800/60 border border-gray-700/50 text-gray-300 px-2.5 py-1 rounded-lg">{plate}</span>
                  ))}
                </div>
              </div>
            )}
          ...
          <div className="glass-card p-5">
            ...
            <div className="space-y-3">
              ...
              <div className="flex justify-between text-sm">
                ...
                <span className="text-gray-200 num">{formatKES(customer.credit_limit)}</span>
              </div>
              {customer.credit_limit > 0 && (
                <>
                  ...
                  <p className="text-xs text-gray-500">Available: {formatKES(Math.max(0, customer.credit_limit - customer.current_balance))}</p>
                </>
              )}
            </div>
          </div>
        ...
        <div className="lg:col-span-2 space-y-5">
                ...
                <tbody>
                  {(transactions?.sales || []).slice(0, 10).map((s, i) => (
                    <motion.tr key={s.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.04 }} className="table-row">
                      ...
                      <td className="table-cell text-right text-sm num"><span className={s.balance_due > 0 ? 'text-red-400' : 'text-gray-500'}>{formatKES(s.balance_due)}</span></td>
                      <td className="table-cell text-center"><StatusBadge status={s.payment_status} size="xs" /></td>
                      <td className="table-cell text-xs text-gray-400">{format(new Date(s.sale_date), 'dd MMM yy')}</td>
                    </motion.tr>
                  ))}
                </tbody>
              </table>
              {(transactions?.sales || []).length === 0 && <div className="text-center py-8 text-gray-500 text-sm">No sales yet</div>}
            </div>
          </div>
          ...
          <div className="glass-card p-5">
            ...
            <div className="overflow-x-auto max-h-72 overflow-y-auto">
              <table className="w-full">
                <thead className="sticky top-0 bg-gray-900"><tr className="border-b border-gray-800/50">
                ...
                <tbody>
                  {(statement?.ledger || []).map((entry, i) => (
                    <motion.tr key={entry.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.02 }} className="table-row">
                      <td className="table-cell text-xs text-gray-400 whitespace-nowrap">{format(new Date(entry.created_at), 'dd MMM yy')}</td>
                      <td className="table-cell text-xs text-gray-300 max-w-[180px] truncate">{entry.description || entry.reference_code || '—'}</td>
                      <td className="table-cell text-right text-xs num"><span className={entry.entry_type === 'debit' ? 'text-red-400 font-medium' : 'text-gray-600'}>{entry.entry_type === 'debit' ? formatKES(entry.amount) : '—'}</span></td>
                      <td className="table-cell text-right text-xs num"><span className={entry.entry_type === 'credit' ? 'text-emerald-400 font-medium' : 'text-gray-600'}>{entry.entry_type === 'credit' ? formatKES(entry.amount) : '—'}</span></td>
              ...
              {(statement?.ledger || []).length === 0 && <div className="text-center py-8 text-gray-500 text-sm">No ledger entries</div>}
            </div>
          </div>
        </div>
      </div>
